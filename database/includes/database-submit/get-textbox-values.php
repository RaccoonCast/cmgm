<?php
if(isset($_GET['carrier'])) $carrier = $_GET['carrier'];
if(isset($_GET['cellsite_type'])) $cellsite_type = $_GET['cellsite_type'];
if(isset($_GET['LTE_1'])) $LTE_1 = $_GET['LTE_1'];
if(isset($_GET['LTE_2'])) $LTE_2 = $_GET['LTE_2'];
if(isset($_GET['LTE_3'])) $LTE_3 = $_GET['LTE_3'];
if(isset($_GET['LTE_4'])) $LTE_4 = $_GET['LTE_4'];
if(isset($_GET['LTE_5'])) $LTE_5 = $_GET['LTE_5'];
if(isset($_GET['LTE_6'])) $LTE_6 = $_GET['LTE_6'];
if(isset($_GET['NR_1'])) $NR_1 = $_GET['NR_1'];
if(isset($_GET['NR_2'])) $NR_2 = $_GET['NR_2'];
if(isset($_GET['pci_match'])) $pci_match = $_GET['pci_match'];
if(isset($_GET['id_pattern_match'])) $id_pattern_match = $_GET['id_pattern_match'];
if(isset($_GET['sector_match'])) $sector_match = $_GET['sector_match'];
if(isset($_GET['bio'])) $bio = $_GET['bio'];
if(isset($_GET['status'])) $status = $_GET['status'];
if(isset($_GET['evidence_link'])) $evidence_link = $_GET['evidence_link'];
if(isset($_GET['address'])) $address = $_GET['address'];
if(isset($_GET['zip'])) $zip = $_GET['zip'];
if(isset($_GET['city'])) $city = $_GET['city'];
if(isset($_GET['state'])) $state = $_GET['state'];
if(isset($_GET['carrier_multiple'])) $carrier_multiple = $_GET['carrier_multiple'];
?>
