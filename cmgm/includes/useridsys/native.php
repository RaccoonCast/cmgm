...
if (!isset($_GET['switchUser'])) {
    // Prepare statement instead of interpolated SQL
    $result = db_stmt($conn, 'SELECT * FROM userID WHERE userID = ?', 's', [$cookie_userID]);
...
if ($userID !== 'guest') {
    ...
    if (!isset($allowGuests) && $curr_userIP != $userIP) {
        ...
        db_stmt($conn, 'UPDATE userID SET userIP = ? WHERE userID = ?', 'ss', [$curr_userIP, $cookie_userID]);
    }
    ...
    if (isset($gmaps_api_key_access)) {
        if ($gmaps_api_key_access == 'true') {
            $maps_api_key = file_get_contents($siteroot . '/secret_maps_api_key.hiddenpass', true);
        }
    }
}
