<?php
    ...
    if ($permit_score >= 75 && ($permit_score < 85)) $ev += 8;
    if ($permit_score >= 85 && ($permit_score < 100)) $ev += 10;
    if ($permit_score >= 100) $ev += 12;

    // trails match
    if ($trails_match >= 35 && ($trails_match < 45)) $ev += 1;
    if ($trails_match >= 45 && ($trails_match < 65)) $ev += 2;
    if ($trails_match >= 65 && ($trails_match < 85)) $ev += 3;
    if ($trails_match >= 85 ) $ev += 4;
    ...
    if ($only_reasonable_location >= 80 && ($only_reasonable_location >= 100)) if ($sector_split_match >= 80 && ($sector_split_match >= 100)) if ($cellmapper_triangulation >= 100 ) $ev += 10;
    if ($only_reasonable_location >= 80 && ($only_reasonable_location >= 100)) if ($sector_split_match >= 0 && ($sector_split_match < 80)) if ($cellmapper_triangulation >= 100 ) $ev += 8;
    if ($only_reasonable_location < 80) if ($sector_split_match >= 80 && ($sector_split_match >= 100)) if ($cellmapper_triangulation >= 100 ) $ev += 6;
    if ($only_reasonable_location < 80) if ($sector_split_match >= 0 && ($sector_split_match < 80)) if ($cellmapper_triangulation >= 100 ) $ev += 5;

    // image evidence
    if ($image_evidence >= 45 && ($image_evidence < 65)) $ev += 4;
    if ($image_evidence >= 65 && ($image_evidence < 75)) $ev += 5;
    if ($image_evidence >= 75 && ($image_evidence < 95)) $ev += 6;
    if ($image_evidence >= 95) $ev += 12;
    ...
    if ($archival_antenna_addition >= 2 && ($archival_antenna_addition < 3)) $ev += 5;
    ...
    if ($permit_score < 40 OR empty($permit_score)) {
      ...
      if ($only_reasonable_location >= 15 && ($only_reasonable_location < 45)) $ev += 2; // (15-55)
      if ($only_reasonable_location >= 45 && ($only_reasonable_location < 65)) $ev += 3; // (55-65)
      if ($only_reasonable_location >= 45 && ($only_reasonable_location < 65)) $ev += 4; // (45-65)
      if ($only_reasonable_location >= 65 && ($only_reasonable_location < 85)) $ev += 5; // (65-85)
      if ($only_reasonable_location >= 85 && ($only_reasonable_location < 95)) $ev += 6; // (85-95)
      if ($only_reasonable_location >= 95 ) $ev += 7;                                    // (95+)
    }
