        ...
        <div class="formsContainerContainer">
            <div id="formsContainer">
                ...
                <select class="misc_cw" title="Set view mode" name="viewMode" id="viewMode">
                    <option value="enbs" <?php if (!isset($_GET['viewMode']) || $_GET['viewMode'] == 'enbs')
                        echo 'selected'; ?>>Mode: eNB Pins</option>
                    <option value="cells" <?php if (isset($_GET['viewMode']) && $_GET['viewMode'] == 'cells')
                        echo 'selected'; ?>>Mode: Cell Polygons</option>
                </select>
    ...
    <script>
        ...
        function updateLabelsOnly() {
            ...
            allVisibleMarkers.forEach((m, index) => {
                const shouldBeVisible = ((map.getZoom() > 10 && index < 250 && labels.checked) || forceLabelVisibility.checked);
        ...
        async function updateData(shouldFetch = true) {
            ...
            if (shouldFetch && unload.checked) { 
                ...
                // 2. Cleanup polygonMap (Only runs in Cell mode)
                if (isCellView) {
