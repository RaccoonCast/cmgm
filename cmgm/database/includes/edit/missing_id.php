...
if (isset($_GET['new'])) {
  ...
  if (!empty($id) && isset($_GET['locsearch'])) {
    ...
    if (!empty($latitude) && !empty($longitude)) @redir("Edit.php?id=" . @mysqli_fetch_array(mysqli_query($conn,"SELECT DISTINCT id, (3959 * ACOS(COS(RADIANS($latitude)) * COS(RADIANS(latitude)) * COS(RADIANS(longitude) - RADIANS($longitude)) + SIN(RADIANS($latitude)) * SIN(RADIANS(latitude)))) AS DISTANCE FROM db HAVING distance < 0.50 ORDER BY distance LIMIT 1"))['id'],"0");
  }
