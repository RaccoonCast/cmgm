...
if (isset($_POST['new'])) {
  $id = mysqli_fetch_array(mysqli_query($conn, "SELECT t1.id + 1 FROM das-db t1 WHERE NOT EXISTS (SELECT * FROM db t2 WHERE t2.id = t1.id + 1) LIMIT 1"))['t1.id + 1'];
  include "includes/editdas/create_new.php";
}
...
if (!isset($status) && !isset($new))) {
  include "includes/editdas/Edit-DB.php";
  die();
}

// Unlock/Lock
//include "includes/edit/lockorunlock.php";
include "includes/editdas/delete.php";
...
// SQL Edit Code
//if ($padlock == "false") include "includes/editas/sql_mgm/the_edit_code.php";
...
<?php
// THE FORM
include "includes/editdas/the_form.php";
$no_edit = "true";
echo '<div class="edit_utilitiy_holder">';
if (!isset($delete) && !isset($new) && !isset($_GET['lock_status']) && $padlock == "false") include "includes/editdas/prev_next.php";
if (!isset($delete) && !isset($new) && !isset($_GET['lock_status']) && $padlock == "false") include "includes/editdas/id_input/footer_search.php";
echo '</div>';
?>
