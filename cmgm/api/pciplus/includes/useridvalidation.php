...
if (isset($_POST['cmgm_session_id'])) {
    $userID = preg_replace("[^_a-zA-Z0-9,-]", "", $_POST['cmgm_session_id']);
    $username = mysqli_fetch_array(mysqli_query($conn, "SELECT username,userID FROM userID WHERE userID='$userID'"))['username'];
    if (empty($username)) error("Invalid user ID.", $cmgm_session_id);
} else {
    if (!isset($userID)) error("User ID not set.","");
    $pciplus_usr = file_get_contents($siteroot . "/secret_pciplus_login_key.hiddenpass", true);
    $tmp_username = mysqli_fetch_array(mysqli_query($conn, "SELECT username,userID FROM userID WHERE userID='$userID'"))['username'];
