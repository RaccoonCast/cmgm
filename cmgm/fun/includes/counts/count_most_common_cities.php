...
while ($row = $result->fetch_assoc()) {
  ...
  if (isset($_GET['percents_view']))  { echo getPercent($row["city_count"]) . "  in " . $url . "<br>";
  } elseif (isset($json_flag))        { $json_array[$location] = $row["city_count"];
  } else                              { echo $row["city_count"] . $plural_namething . "  in " . $url . "<br>";
  }
